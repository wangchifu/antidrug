...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            {{ Form::open(['route' => ['telephone_propagandas.update',$telephone_propaganda->id], 'method' => 'post', 'files' => true]) }}
            <div class="form-group">
                <label for="title">宣導主題<strong class="text-danger">*</strong></label>
                <input type="text" class="form-control" name="title" id="title" required value="{{ $telephone_propaganda->title }}">
            </div>
            <div class="form-group">
                <label for="date">宣導日期<strong class="text-danger">*</strong>
                    <small class="text-secondary">(若有多個場次，請輸入第一場次的日期)</small>
                </label>
                <input type="date" class="form-control" name="date" id="date" required maxlength="10" value="{{ $telephone_propaganda->date }}">
            </div>
            <div class="form-group">
                <label for="object">宣導對象<strong class="text-danger">*</strong></label><br>
                {{ Form::select('object',$objects,$telephone_propaganda->object,['id'=>'object','class' => 'form-control','required'=>'required']) }}
                <br><br>
            </div>
            <div class="form-group">
                <label for="type">宣導類別<strong class="text-danger">*</strong></label><br>
                {{ Form::select('type',$types,$telephone_propaganda->type,['id'=>'type','class' => 'form-control','required'=>'required']) }}
                <br><br>
            </div>
            <div class="form-group">
                <label for="personnel">宣導人員<strong class="text-danger">*</strong></label>
                <input type="text" class="form-control" name="personnel" id="personnel" required value="{{ $telephone_propaganda->personnel }}">
            </div>
            <div class="form-group">
                <label for="place">宣導地點<strong class="text-danger">*</strong></label>
                <input type="text" class="form-control" name="place" id="place" required value="{{ $telephone_propaganda->place }}">
            </div>
            <div class="form-group">
                <label for="person_times">宣導人次<strong class="text-danger">*</strong>
                <small class="text-secondary">(若有多個場次，請輸入加總人數)</small>
                </label>
                <input type="number" class="form-control" name="person_times" id="person_times" required value="{{ $telephone_propaganda->person_times }}">
            </div>
            <div class="form-group">
                <label for="times">宣導場次<strong class="text-danger">*</strong></label><br>
                {{ Form::select('times',$times,$telephone_propaganda->times,['id'=>'times','class' => 'form-control','required'=>'required']) }}
                <br><br>
            </div>
            <div class="form-group">
                <label for="content">宣導內容<strong class="text-danger">*</strong></label>
                <textarea name="content" id="content" required class="form-control">{{ $telephone_propaganda->content }}</textarea>
            </div>
            <div class="form-group">
                <label for="result">宣導成效<strong class="text-danger">*</strong></label>
                <input type="text" class="form-control" name="result" id="result" required value="{{ $telephone_propaganda->result }}">
            </div>
            <div class="form-group">
                <label for="money_source">經費來源</label>
                <input type="text" class="form-control" name="money_source" id="money_source" value="{{ $telephone_propaganda->money_source }}">
            </div>
            <div class="form-group">
                <label for="money">經費總額</label>
                <input type="number" class="form-control" name="money" id="money" value="{{ $telephone_propaganda->money }}">
            </div>
            <a name="pic_group"></a>
            <div class="form-group">
                <label>
                        ...
                        @foreach($pics as $pic)
                            @if(file_exists(storage_path('app/public/telephone_propagandas/'.$telephone_propaganda->id.'/'.$pic->pic)))
                                <div class="col-3" style="margin: 5px">
                                    <figure class="figure">
                                        <a href="{{ route('telephone_propagandas.del_pic',$pic->id) }}" onclick="return confirm('確定刪除？')"><i class="fas fa-times-circle text-danger"></i></a>
