    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            @if(auth()->user()->admin==1)
                <li class="breadcrumb-item"><a href="{{ route('parent_propagandas.review') }}">「家長宣導」管理列表</a></li>
            @else
                <li class="breadcrumb-item"><a href="{{ route('parent_propagandas.index') }}">「家長宣導」列表</a></li>
            @endif
            <li class="breadcrumb-item active" aria-current="page">{{ $parent_propaganda->title }}</li>
        </ol>
    </nav>
    @endauth
    <a href="#" onclick="history.back()" class="btn btn-secondary btn-sm"><i class="fas fa-backward"></i> 返回</a>
    <a href="{{ route('parent_propagandas.print',$parent_propaganda->id) }}" class="btn btn-primary btn-sm" target="_blank"><i class="fas fa-print"></i> 列印</a>
            ...
            @foreach($pics as $pic)
                @if(file_exists(storage_path('app/public/parent_propagandas/'.$parent_propaganda->id.'/'.$pic->pic)))
                    <div class="col-5" style="margin: 5px">
                        <figure class="figure">
                            <a href="{{ asset('storage/parent_propagandas/'.$parent_propaganda->id.'/'.$pic->pic) }}" class="venobox" data-gall="gall1">
